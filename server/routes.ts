...
export function registerRoutes(app: Express): Server {
  ...
  // NEW: Test endpoint to verify code updates
  ...
  app.get("/api/metrics/historical", async (req, res) => {
    console.log('HISTORICAL ENDPOINT HIT!');
    try {
      const filter = (req.query.filter as 'total' | 'sfr' | 'mf') || 'total';
      console.log(`Fetching historical metrics with filter: ${filter}`);
      
      // For now, return simple synthetic trends to test the frontend
      const trends = {
        occupancy: {
          ytd: 82.1,
          lastMonth: 83.8,
      ...
      console.log('Historical trends result:', JSON.stringify(trends, null, 2));
      res.json(trends);
    } catch (error) {
